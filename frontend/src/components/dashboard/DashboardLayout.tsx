...
const SidebarContent = ({ isFarmer, navItems, pathname, user, logout, setIsMobileMenuOpen }: SidebarContentProps) => {
      ...
      <div className="p-6 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-foreground/60 mt-1 font-medium uppercase tracking-wider">
            {isFarmer ? d('farmerSpace') : d('collectorSpace')}
          </p>
        </div>
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const basePath = isFarmer ? '/dashboard/agriculteur' : '/dashboard/collecteur';

  const navItems = isFarmer ? [
        { name: d('overview'), href: `${basePath}`, icon: faHome },
        { name: d('myOffers'), href: `${basePath}/offres`, icon: faList },
        { name: d('matches'), href: `${basePath}/matches`, icon: faHandshake },
        { name: d('receivedOffers'), href: `${basePath}/purchases/received`, icon: faShoppingBag },
        { name: d('community'), href: `/dashboard/community`, icon: faUsers },
        { name: d('chat'), href: `/dashboard/chat`, icon: faEnvelope, badgeCount: unreadMessages },
        { name: d('notifications'), href: `/dashboard/notifications`, icon: faBell, badgeCount: unreadNotifications },
        { name: t('profile'), href: `/profile`, icon: faUser },
      ]
    : [
        { name: d('overview'), href: `${basePath}`, icon: faHome },
        { name: m('title'), href: `${basePath}/marche`, icon: faStore },
        { name: d('myDemands'), href: `${basePath}/demandes`, icon: faList },
        { name: d('matches'), href: `${basePath}/matches`, icon: faHandshake },
        { name: d('sentPurchases'), href: `${basePath}/purchases/sent`, icon: faShoppingCart },
        { name: d('community'), href: `/dashboard/community`, icon: faUsers },
        { name: d('chat'), href: `/dashboard/chat`, icon: faEnvelope, badgeCount: unreadMessages },
        { name: d('notifications'), href: `/dashboard/notifications`, icon: faBell, badgeCount: unreadNotifications },
        { name: t('profile'), href: `/profile`, icon: faUser },
